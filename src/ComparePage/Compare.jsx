...
function ScoreRing({ score, size = 48 }) {
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <span className="absolute inset-0 flex items-center justify-center font-sora font-semibold text-[13px] text-[#0F172A]">{score}</span>
    </div>
  );
}
    ...
    >
      <div className="flex flex-col xl:flex-row bg-[#FFFFFF] rounded-[20px] border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="flex-1 p-4 sm:p-5 xl:p-6 flex flex-col">
          <div className="flex overflow-x-auto items-center gap-3 text-[12px] font-inter font-normal text-[#94A3B8] mb-4 pb-1 whitespace-nowrap [&::-webkit-scrollbar]:hidden">
          ...
          <div className="mb-5">
            ...
            <div className="flex overflow-x-auto gap-2 pb-1 [&::-webkit-scrollbar]:hidden">
              {tags.map((tag, i) => (
                <span key={tag} className={`shrink-0 px-2.5 py-1 sm:py-1.5 rounded-[6px] text-[11px] sm:text-[12px] font-inter font-medium border ${i < 2 ? "bg-[#F8FAFC] text-[#475569] border-slate-200" : "bg-white text-[#0F172A] border-slate-200 shadow-sm"}`}>
                  {tag}
                </span>
              ))}
            </div>
            ...
            <motion.div variants={cardVars} className="shrink-0 w-[220px] md:w-auto p-4 rounded-[14px] bg-white border border-slate-200 flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-baseline gap-1.5">
                  <span className="text-[18px] font-sora font-bold text-[#0F172A] leading-[1.3] tracking-[-0.02em]">156</span>
            ...
            <motion.div variants={cardVars} className="shrink-0 w-[220px] md:w-auto p-4 rounded-[14px] bg-white border border-slate-200 flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center gap-2">
                  <span className="text-[16px] md:text-[18px] font-sora font-[800] text-[#0F172A] leading-[1.2] tracking-[-0.02em]">₹67,499</span>
                </div>
              </div>
            </motion.div>
            <motion.div variants={cardVars} className="shrink-0 w-[220px] md:w-auto p-4 rounded-[14px] bg-white border border-slate-200 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[11px] font-inter font-semibold text-[#94A3B8] uppercase tracking-[0.08em]">You Save</span>
                </div>
                <div className="flex items-baseline gap-1.5">
                  <span className="text-[18px] font-sora font-[800] text-[#0F172A] leading-[1.2] tracking-[-0.02em]">₹2,500</span>
                  <span className="text-[#10B981] text-[12px] font-inter font-medium">3.6% ↓</span>
                </div>
              </div>
            </motion.div>
            <motion.div variants={cardVars} className="shrink-0 w-[220px] md:w-auto p-4 rounded-[14px] bg-white border border-slate-200 flex flex-col justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[11px] font-inter font-semibold text-[#94A3B8] uppercase tracking-[0.08em]">Bulk Advantage</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[17px] font-sora font-bold text-[#0F172A] leading-[1.3] tracking-[-0.02em]">MOQ 10</span>
                </div>
              </div>
            </motion.div>
          </motion.div>
            ...
            <div className="flex flex-row items-center justify-between gap-3 flex-wrap">
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center h-10 rounded-[10px] bg-white border border-slate-200 shadow-sm overflow-hidden shrink-0">
                  <button onClick={() => setQty(Math.max(10, qty - 1))} className="w-10 h-full flex items-center justify-center text-[#475569] hover:bg-[#F8FAFC] font-sora font-semibold text-[13px]">−</button>
              ...
              <div className="text-right">
                <p className="text-[11px] font-inter font-semibold text-[#94A3B8] uppercase tracking-[0.08em] mb-1">Est. Value</p>
        ...
        <div className="w-full xl:w-[320px] bg-[#F8FAFC] border-t xl:border-t-0 xl:border-l border-slate-200 p-4 sm:p-5 xl:p-6 flex flex-col justify-between">
                ...
                <div key={s.name} className="group cursor-pointer">
                  <div className="flex justify-between items-center mb-1.5">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[13px] font-inter font-medium text-[#0F172A] group-hover:text-[#EC4899] transition-colors truncate max-w-[150px]">{s.name}</span>
                      {s.verified && <CheckCircle2 size={12} className="text-[#10B981] shrink-0" />}
                    </div>
                    <span className="text-[14px] font-sora font-[800] text-[#0F172A] tracking-[-0.02em] shrink-0">{s.price}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] font-sora font-semibold text-[#475569] w-7 text-right shrink-0">{s.match}%</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mt-5 pt-4 border-t border-slate-200">
            <GradientButton className="w-full group flex items-center justify-between py-2.5 px-4 rounded-[12px] bg-white border border-slate-200 shadow-sm hover:border-slate-300 hover:shadow transition-all text-[13px] font-inter font-semibold">
              <div className="flex items-center gap-2">
                <ShieldCheck size={14} />
                <span>Compare 156 Suppliers</span>
              </div>
    ...
    >
      ...
      <td className="py-2 px-4 align-middle border-l border-slate-100">
        <div className="flex flex-col">
          <span className="text-[11px] font-inter font-semibold text-[#94A3B8] mt-1 tracking-[0.08em] uppercase">Min {supplier.moq} Units</span>
      ...
      <td className="py-2 px-4 align-middle bg-[#F8FAFC]/50">
        <div className="flex flex-col relative group/tooltip">
          ...
          <span className="text-[16px] font-sora font-[800] text-[#10B981] tabular-nums tracking-[-0.02em]">{inr(dynamicPrice)}</span>
          
          {/* Tooltip */}
          <div className="absolute bottom-full left-0 mb-2 hidden group-hover/tooltip:block w-48 p-2 bg-[#0F172A] text-white text-[12px] font-inter font-normal rounded-lg shadow-lg z-50">
      ...
      <td className="py-2 px-4 align-middle border-r border-slate-100">
        <div className="flex flex-col items-start">
          <span className="text-[11px] font-inter font-semibold text-[#94A3B8] uppercase tracking-[0.08em] mb-1">Bulk {supplier.bulkDeal.qty}+ Units</span>
          <span className="text-[14px] font-sora font-[800] text-[#0F172A] mb-1 tracking-[-0.02em]">{inr(supplier.bulkDeal.price)}</span>
      ...
      <td className="py-2 px-4 align-middle">
        <div className="flex flex-col">
          <span className="flex items-center gap-1.5 text-[13px] font-sora font-medium text-[#0F172A]">
            ⭐ {supplier.rating}
          </span>
      ...
      <td className="py-2 px-4 align-middle">
        <span className="flex items-center gap-2 text-[13px] font-inter font-medium text-[#0F172A]">
      ...
      <td className="py-2 px-4 align-middle">
        <div className="flex flex-col">
          <span className={`text-[14px] font-sora font-medium ${supplier.stock < 100 ? 'text-[#F97316]' : 'text-[#0F172A]'}`}>
            {supplier.stock.toLocaleString()} <span className="text-[13px] font-inter text-[#94A3B8] font-normal ml-0.5">{supplier.stockUnit}</span>
          </span>
          {supplier.stock < 100 && (
      ...
      <td className="py-2 pl-4 pr-8 align-middle text-right">
        <div className="flex items-center justify-end gap-2 ">
          <SecondaryButton 
            ...
            className="h-10 px-3 rounded text-[13px] font-semibold border-slate-200 hover:border-slate-300"
          >
    ...
    <div className="w-full flex flex-col mt-4">
          ...
          <thead className="sticky top-0 z-30 table-header-shadow">
            <tr className="bg-[#F8FAFC] border-y border-slate-200">
              {COL_HEADERS.map((h, i) => (
                <th 
                  key={h.label} 
                  className={`py-4 px-4 ${i === 0 ? 'pl-8' : ''} ${i === COL_HEADERS.length - 1 ? 'pr-8' : ''} text-[11px] font-inter font-semibold uppercase tracking-[0.08em] text-[#94A3B8] whitespace-nowrap bg-[#F8FAFC] backdrop-blur-md ${h.w}`}
                >
                  {h.label}
                </th>
              ))}
      ...
      <div className="px-6 md:px-8 py-5 bg-[#FDFDFE] flex flex-col sm:flex-row items-center justify-between gap-4 rounded-b-[28px]">
        <span className="text-[13px] font-inter font-normal text-[#475569]">
          Showing <strong className="text-[#0F172A] font-medium">{visibleCount}</strong> of 156 verified suppliers
        </span>
        
        {visibleCount < 10 ? (
          <button 
            onClick={handleLoadMore}
            className="flex items-center gap-2 px-4 py-2 rounded-[12px] bg-white border border-slate-200 text-[13px] font-inter font-semibold text-[#0F172A] hover:bg-[#F8FAFC] hover:shadow-sm transition-all"
          >
            View All Seller <ChevronDown size={16} className="text-[#94A3B8]" />
          </button>
        ) : (
          <button className="flex items-center gap-2 px-4 py-2 rounded-[12px] bg-white border border-slate-200 text-[13px] font-inter font-semibold text-[#0F172A] hover:bg-[#F8FAFC] hover:shadow-sm transition-all"
  ...
  return (
    <div className="rounded-[20px] bg-white border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow">
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <span className="text-[11px] font-inter font-semibold uppercase tracking-[0.08em] text-[#94A3B8]">{label}</span>
      </div>
      <p className="text-[16px] font-sora font-semibold text-[#0F172A]">{value}</p>
    </div>
  );
}
...
>
  ...
  <div className="px-5 py-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center rounded-t-xl">
    <div className="flex items-center gap-3">
      ...
      <div>
        <h2 className="text-[16px] font-semibold text-slate-900 leading-[1.35]">{supplier.name}</h2>
  ...
  <div className="p-5 flex flex-col gap-5">
    ...
    <div className="border border-slate-200 rounded-lg bg-white shadow-sm overflow-hidden">
      <div className="grid grid-cols-2 divide-x divide-slate-100 p-4">
        <div>
          <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-[0.08em] mb-1">Base Unit Price</p>
          <div className="flex items-baseline gap-1">
            <h1 className="text-[24px] md:text-[26px] font-[800] text-slate-900 tracking-[-0.03em] leading-[1.2]">{inr(supplier.moqPrice)}</h1>
            <span className="text-[13px] text-slate-500 font-normal">/ unit</span>
          </div>
        ...
        <div className="pl-4">
          ...
          <h1 className="text-[16px] font-semibold text-slate-900 tracking-[-0.02em]">{supplier.stock}</h1>
      ...
      <div className="bg-slate-50 border-t border-slate-100 px-4 py-2.5 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <span className="bg-green-100 text-green-700 px-1.5 py-0.5 rounded text-[10px] font-semibold uppercase tracking-[0.10em]">Deal</span>
          <span className="text-slate-700 text-[13px] font-medium">{supplier.bulkDeal.qty} Units @ {inr(supplier.bulkDeal.price)}</span>
        </div>
        <span className="text-green-600 font-semibold text-[12px]">Save {inr(supplier.bulkDeal.saving)}</span>
      </div>
    </div>

    {/* Supplier Metrics Grid */}
    <div>
      <h3 className="text-[11px] font-semibold text-slate-900 mb-2 uppercase tracking-[0.08em]">Supplier Metrics</h3>
      <div className="grid grid-cols-4 gap-2">
        ...
        <div className="border border-slate-100 rounded-md bg-slate-50 p-2.5 flex flex-col gap-1 text-left">
          <span className="text-[11px] font-normal text-slate-500 flex items-center gap-1"><Package size={10} /> Completed</span>
          <span className="text-[13px] font-medium text-slate-900">{details.orders}</span>
        </div>
        <div className="border border-slate-100 rounded-md bg-slate-50 p-2.5 flex flex-col gap-1 text-left">
          <span className="text-[11px] font-normal text-slate-500 flex items-center gap-1"><MessageCircle size={10} /> Response</span>
          <span className="text-[13px] font-medium text-slate-900">{details.responseRate}</span>
        </div>
        <div className="border border-slate-100 rounded-md bg-slate-50 p-2.5 flex flex-col gap-1 text-left">
          <span className="text-[11px] font-normal text-slate-500 flex items-center gap-1"><Clock size={10} /> On-Time</span>
          <span className="text-[13px] font-medium text-slate-900">{details.onTime}</span>
        </div>
        <div className="border border-slate-100 rounded-md bg-slate-50 p-2.5 flex flex-col gap-1 text-left">
          <span className="text-[11px] font-normal text-slate-500 flex items-center gap-1"><Shield size={10} /> Warranty</span>
          <span className="text-[13px] font-medium text-slate-900">{details.warranty}</span>
        </div>
      </div>
    </div>
    ...
    <div className="flex gap-2">
      <span className="px-2 py-1 rounded text-[10px] font-semibold uppercase tracking-[0.10em] bg-emerald-50 text-emerald-700 border border-emerald-100">Verified Partner</span>
      <span className="px-2 py-1 rounded text-[10px] font-semibold uppercase tracking-[0.10em] bg-blue-50 text-blue-700 border border-blue-100">Secure Checkout</span>
...
function NegotiationCRM({ qty }) {
  ...
  const input =
    "w-full h-[52px] md:h-[56px] px-4 md:px-5 rounded-[14px] bg-white border border-slate-200 outline-none text-[14px] font-inter font-normal text-[#0F172A] placeholder:text-[13px] placeholder:font-normal transition-all focus:border-[#EC4899] focus:ring-2 focus:ring-pink-500/10";
  ...
  return (
    ...
    >
      ...
      >
        ...
        placeholder="Mention payment terms, delivery requirements, warranty, customization..."
        className="
        w-full
        px-4
        md:px-5
