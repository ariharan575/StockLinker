      ...
      <AnimatePresence mode="wait">
        ...
        {isLoading && (
          ...
          >
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="animate-pulse rounded-[24px] bg-white border border-slate-100 p-2.5 shadow-sm"
              >
                <div className="w-full h-[78px] rounded-[18px] bg-slate-200 mb-3" />
                <div className="h-3 w-3/4 bg-slate-200 rounded-full mb-2 mx-auto" />
                <div className="h-4 w-1/2 bg-emerald-50 rounded-full mx-auto mt-2" />
              </div>
            ))}
          </motion.div>
        )}
        ...
        {!isLoading && error && (
          <motion.div
            ...
            className="flex flex-col items-center justify-center p-8 rounded-3xl border border-red-100 bg-red-50/50"
          >
            <AlertCircle className="w-8 h-8 text-red-400 mb-3" />
            <p className="text-sm font-semibold text-red-600 mb-1">Oops! Something went wrong.</p>
            <button 
        ...
        {!isLoading && !error && categories.length === 0 && (
          <motion.div
            ...
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
            className="flex flex-col items-center justify-center p-12 rounded-3xl border-2 border-dashed border-slate-200 bg-white"
          >
            ...
            {categories.map((cat, i) => (
              ...
              >
                ...
                <div className="relative z-10">
                  <div className="w-full h-[78px] rounded-[18px] overflow-hidden bg-slate-100 border border-slate-100 mb-3 shadow-sm flex items-center justify-center">
