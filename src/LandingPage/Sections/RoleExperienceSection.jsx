...
const TiltCard = ({ children, className, glowColor }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative h-full w-full rounded-[32px] bg-white/70 backdrop-blur-2xl border border-white shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-shadow duration-500 hover:shadow-[0_20px_40px_rgb(0,0,0,0.08)] overflow-hidden flex flex-col ${className}`}
    >
...
// --- WORLD CLASS PREMIUM FEATURE ANIMATIONS ---
const featuresStaggerContainer = {
  hidden: { opacity: 0 },
  visible: {
    opacity: 1,
    transition: { 
      staggerChildren: 0.06, // Fast, silky cascade
      delayChildren: 0.3,    // Wait for the panel to slightly enter
    }
  }
};
...
const premiumIconReveal = {
  ...
  visible: { 
    ...
    transition: { 
      ...
      damping: 16,
      mass: 1,
      delay: 0.05 // Tiny micro-delay so the text moves first, then icon pops
    } 
  }
};
...
const PremiumFeature = ({ text, theme }) => {
  ...
  return (
    <motion.div 
      ...
      whileHover="hover"
      className="flex items-center gap-3 sm:gap-4 w-full cursor-default"
    >
      <motion.div 
      ...
      >
        ...
        >
          <motion.div variants={iconHoverVariants} className="flex items-center justify-center w-full h-full">
            <Check size={14} strokeWidth={3} className="text-white" />
          </motion.div>
        </motion.div>
        <span className="text-[14px] md:text-[15px] lg:text-[16px] font-medium text-slate-700 tracking-tight lg:whitespace-nowrap whitespace-normal transition-colors duration-200">
          {text}
        </span>
      </motion.div>
    </motion.div>
          ...
          {index < steps.length - 1 && (
            <div className="flex-1 h-[2px] mx-2 sm:mx-4 bg-slate-100 relative top-[-10px] rounded-full overflow-hidden">
              <motion.div 
                ...
                viewport={{ once: true }}
                transition={{ duration: 1, delay: 0.8 + (index * 0.2), ease: "easeInOut" }}
              />
            </div>
          )}
        </React.Fragment>
