...
const handleRoleSubmit = async (e) => {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-gray-200 font-sans text-slate-800 flex items-center justify-center px-4 py-6 sm:p-8 antialiased">
      ...
      <motion.div initial="hidden" animate="visible" variants={containerVariants} className="relative z-10 w-full max-w-3xl overflow-hidden rounded-[24px] border border-slate-200/80 bg-white/95 p-5 sm:p-8 md:p-10 shadow-[0_20px_50px_-12px_rgba(15,23,42,0.06)] backdrop-blur-md">
        ...
        <div className="absolute inset-x-0 top-0 h-[1px] bg-gradient-to-r from-transparent via-white to-transparent opacity-70" />
           ...
           <motion.div variants={itemVariants} whileHover={{ scale: 1.03, y: -0.5 }} className="flex w-[135px] items-start gap-1.5 rounded-full border border-slate-200/60 bg-slate-50 px-3 py-1 shadow-sm">
        ...
        <form onSubmit={handleRoleSubmit} className="mt-6 sm:mt-8 space-y-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5">
            ...
            {/* SHOPKEEPER CARD */}
            <motion.div variants={itemVariants} whileHover={loading ? {} : { y: -3, scale: 1.005 }} whileTap={loading ? {} : { scale: 0.995 }} onClick={() => !loading && setSelectedRole("shopkeeper")} className={`group relative cursor-pointer overflow-hidden rounded-[20px] border p-5 transition-all duration-300 ${selectedRole === "shopkeeper" ? "border-pink-500 bg-white shadow-[0_12px_30px_rgba(244,63,94,0.06)] ring-1 ring-pink-500/10" : "border-slate-200 bg-white/60 hover:border-slate-300 hover:bg-white"}`}>
              ...
              <div className="relative flex items-start gap-4">
                <motion.div whileHover={loading ? {} : { rotate: [-3, 3, -3], scale: 1.03 }} transition={{ duration: 0.4 }} className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-[14px] transition-all duration-300 ${selectedRole === "shopkeeper" ? "bg-gradient-to-tr from-pink-500 to-rose-500 text-white shadow-md shadow-pink-500/10" : "bg-slate-100 text-slate-600"}`}>
                  <Store className="h-5 w-5" />
                </motion.div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="text-base font-bold tracking-tight text-slate-900">Shopkeeper</h3>
                  ...
                  <div className="mt-3 space-y-1.5">
                    <motion.div whileHover={{ x: 1 }} className="flex items-center gap-2 text-xs font-semibold text-slate-500">
            ...
            {/* WHOLESALER CARD */}
            <motion.div variants={itemVariants} whileHover={loading ? {} : { y: -3, scale: 1.005 }} whileTap={loading ? {} : { scale: 0.995 }} onClick={() => !loading && setSelectedRole("wholesaler")} className={`group relative cursor-pointer overflow-hidden rounded-[20px] border p-5 transition-all duration-300 ${selectedRole === "wholesaler" ? "border-pink-500 bg-white shadow-[0_12px_30px_rgba(244,63,94,0.06)] ring-1 ring-pink-500/10" : "border-slate-200 bg-white/60 hover:border-slate-300 hover:bg-white"}`}>
              ...
              <div className="relative flex items-start gap-4">
                <motion.div whileHover={loading ? {} : { rotate: [-3, 3, -3], scale: 1.03 }} transition={{ duration: 0.4 }} className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-[14px] transition-all duration-300 ${selectedRole === "wholesaler" ? "bg-gradient-to-tr from-pink-500 to-rose-500 text-white shadow-md shadow-pink-500/10" : "bg-slate-100 text-slate-600"}`}>
                  <Truck className="h-5 w-5" />
                </motion.div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    <h3 className="text-base font-bold tracking-tight text-slate-900">Wholesaler</h3>
                    <AnimatePresence>
          ...
          <motion.div variants={itemVariants} className="space-y-4 pt-1">
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }} className="flex items-center justify-center gap-1.5 text-center text-[11px] sm:text-xs text-slate-400/90">
