...
function SupplierCard({ supplier, index, isConnected, onAccept, isPendingReq }) {
  ...
  return (
    <motion.div {...fadeUp(index * 0.05)} whileHover={{ y: -4, boxShadow: "0 20px 40px -10px rgba(15,23,42,0.1)" }} className="bg-white rounded-2xl p-5 border border-slate-200 transition-all flex flex-col h-full relative" style={{ boxShadow: "0 4px 20px -5px rgba(15,23,42,0.05)" }}>
      ...
      <div className="flex flex-wrap gap-1.5 mb-3"><span className="flex items-center gap-1 text-xs text-zinc-600 bg-zinc-50 px-2 py-1 rounded-md border border-zinc-100"><MapPin size={12}/> {supplier.location}</span></div>
      <div className="flex items-center justify-between bg-zinc-50 border border-zinc-100 rounded-lg px-3 py-2.5 mb-3">
        <div className="flex items-center gap-1.5"><MapPin size={12} className="text-zinc-400"/><span className="text-[11px] text-zinc-500 font-medium">Radius:</span><span className="text-xs font-bold text-zinc-900">{supplier.deliveryRadius || "Not Set"}</span></div>
        <div className="w-px h-4 bg-zinc-200"></div>
        <div className="flex items-center gap-1.5"><Truck size={12} className="text-zinc-400"/><span className="text-[11px] text-zinc-500 font-medium">Est:</span><span className="text-xs font-bold text-zinc-900">{supplier.deliveryEstimate || "Standard"}</span></div>
      </div>
...
export default function ConnectedSupplierPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 min-h-screen bg-[#FAFAFA]" style={{ fontFamily: "'Inter', sans-serif" }}>
      <AnimatePresence><LiveToast notification={liveToast} onClose={() => setLiveToast(null)} /></AnimatePresence>

      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <motion.h1 {...fadeUp(0)} className="text-3xl font-extrabold text-slate-900 tracking-tight">My Network</motion.h1>
